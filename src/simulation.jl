...
function Model(
    ...
    for I in CartesianIndices(weir_area[:, :, 1])
        ...
        column =
            SoilColumn(x[I[1]], y[I[2]], domain.z, domain.Δz, g_column, c_column, o_column)
    ...
    output = Output(fill(NaN, shape), fill(NaN, shape), fill(NaN, shape), fill(NaN, shape))
...
function advance_forcingperiod!(model, forcings, duration)
    ...
    for (I, column) in zip(model.index, model.columns)
        ...
        # Compute
        phreatic, subsidence, consolidation, oxidation =
...
function advance_forcingperiod!(simulation)
    ...
    advance_forcingperiod!(simulation.model, forcings, periodduration(clock))
